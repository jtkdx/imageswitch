...
class ImageSwitcher(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.img = None  # 現在表示されている画像の参照を保持
    ...
    def show_image(self, tk_img, img):
        if self.image_window is None or not self.image_window.winfo_exists():
            ...
            # 拡大ボタンの追加
            self.zoom_in_button = tk.Button(self.image_window, text="拡大", command=self.zoom_in)
            self.zoom_in_button.pack(side=tk.LEFT, padx=5)

            # 縮小ボタンの追加
            self.zoom_out_button = tk.Button(self.image_window, text="縮小", command=self.zoom_out)
    ...
    def zoom_in(self):
        # 画像を拡大
        self.img = self.img.resize((int(self.img.width * 1.2), int(self.img.height * 1.2)), Image.Resampling.LANCZOS)
    ...
    def zoom_out(self):
        # 画像を縮小
        self.img = self.img.resize((int(self.img.width * 0.8), int(self.img.height * 0.8)), Image.Resampling.LANCZOS)
        self.tk_img = ImageTk.PhotoImage(self.img)
        self.image_label.configure(image=self.tk_img)
        self.image_label.image = self.tk_img
